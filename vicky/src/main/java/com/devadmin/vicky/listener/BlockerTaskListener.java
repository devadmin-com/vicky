...
/**
 * When a blocker task is assigned to a user, send them a private message. (Implements Story TL-105
 * issue assigned -> slack private message)
 *
 * <p>don't send notification if creator = assignee editor = assignee commenter = assignee mention
 * in comment = assignee
 */
@Component
public class BlockerTaskListener extends TaskToMessageListener {
  ...
  public void onApplicationEvent(TaskEventModelWrapper eventWrapper) {
    ...
    if (task.getPriority() == TaskPriority.BLOCKER) {
      ...
      BlockerTaskTracker tracker =
          new BlockerTaskTracker(event, blockerMessage, messageService, assignedTo);
