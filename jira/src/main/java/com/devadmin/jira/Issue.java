...
import com.devadmin.jira.util.WorklogUtils;
import java.io.File;
...
public class Issue extends Resource {
  ...
  public static final class FluentCreate {
     ...
     *
     * @param includedFields Specifies which issue fields will be included in the result. <br>
     *     Some examples how this parameter works:
     *     <ul>
     *       <li>*all - include all fields
     *       <li>*navigable - include just navigable fields
     *       <li>summary,comment - include just the summary and comments
     *       <li>*all,-comment - include all fields
     *     </ul>
     *
     * @throws JiraException when the create fails
     */
    ...
    private Issue executeCreate(String includedFields) throws JiraException {
      ...
      if (!(result instanceof JSONObject)
          || !((JSONObject) result).containsKey("key")
  ...
  public static final class FluentRemoteLink {
    ...
    /**
     * A globally unique identifier which uniquely identifies the remote application and the remote
     * object within the remote system. The maximum length is 255 characters. This call sets also
     * the {@link #url(String) url}.
     *
     ...
     * @param title Text for the tooltip of the main icon describing the type of the object in the
     *     remote system.
     * @return this instance
     */
     ...
     * @param title Text for the tooltip of the main icon describing the type of the object in the
     *     remote system.
     * @param iconUrl Text for the tooltip of the main icon describing the type of the object in the
     *     remote system.
     * @param statusUrl A hyperlink for the tooltip of the the status icon.
     * @return this instance
     */
    public FluentRemoteLink status(
        final boolean resolved, final String iconUrl, final String title, final String statusUrl) {
    ...
    /**
     * Relationship between the remote object and the JIRA issue. This can be a verb or a noun. It
     * is used to group together links in the UI.
     *
    ...
    /**
     * The application for this remote link. Links are grouped on the type and name in the UI. The
     * name-spaced type of the application. It is not displayed to the user. Renderering plugins can
     * register to render a certain type of application.
     *
     * @param type The name-spaced type of the application.
     * @param name The human-readable name of the remote application instance that stores the remote
     *     object.
     * @return this instance
     */
    ...
    /**
     * Creates or updates the remote link if a {@link #globalId(String) global id} is given and
     * there is already a remote link for the specified global id.
     *
  ...
  public final class FluentUpdate {
    ...
    public void execute() throws JiraException {
      ...
      if (fieldmap.size() > 0) req.put("fields", fieldmap);

      if (updatemap.size() > 0) req.put("update", updatemap);
    ...
    private FluentUpdate fieldOperation(String oper, String name, Object value) {
      if (!fieldOpers.containsKey(name)) fieldOpers.put(name, new ArrayList());
  ...
  /**
   * Iterates over all issues in the query by getting the next page of issues when the iterator
   * reaches the last of the current page.
   */
  private static class IssueIterator implements Iterator<Issue> {
    ...
    /**
     * Gets the next issue, returning null if none more available Will ask the next set of issues
     * from the server if the end of the current list of issues is reached.
     *
    ...
    /**
     * Execute the query to get the next set of issues. Also sets the startAt, maxMresults, total
     * and issues fields, so that the SearchResult can access them.
     *
   ...
   *
   * <p>The issues of the result can be retrived from this class in 2 ways.
   *
   * <p>The first is to access the issues field directly. This is a list of Issue instances. Note
   * however that this will only contain the issues fetched in the initial search, so its size will
   * be the same as the max result value or below.
   *
   * <p>The second way is to use the iterator methods. This will return an Iterator instance, that
   * will iterate over every result of the search, even if there is more than the max result value.
   * The price for this, is that the call to next has none determistic performence, as it sometimes
   * need to fetch a new batch of issues from Jira.
   */
...
//    security = Field.getResource(Security.class, fields.get(Field.SECURITY), restclient);
  ...
  /**
   * Adds a remote link to this issue. At least set the {@link FluentRemoteLink#url(String) url} or
   * {@link FluentRemoteLink#globalId(String) globalId} and {@link FluentRemoteLink#title(String)
   * title} before {@link FluentRemoteLink#create() creating} the link.
   *
  ...
  public WorkLog addWorkLog(String comment, DateTime startDate, long timeSpentSeconds)
      throws JiraException {
    try {
      if (comment == null) throw new IllegalArgumentException("Invalid comment.");
      if (startDate == null) throw new IllegalArgumentException("Invalid start time.");
      ...
      req.put(
          "started", DateTimeFormat.forPattern(Field.DATETIME_FORMAT).print(startDate.getMillis()));
  ...
  private static JSONObject realGet(
      RestClient restclient, String key, Map<String, String> queryParams) throws JiraException {
  ...
  public static Issue get(
      RestClient restclient, String key, final String includedFields, final String expand)
      throws JiraException {
  ...
  /**
   * Search for issues with the given query and specify which fields to retrieve. If the total
   * results is bigger than the maximum returned results, then further calls can be made using
   * different values for the <code>startAt</code> field to obtain all the results.
   *
   * @param restclient REST client instance
   * @param jql JQL statement
   * @param includedFields Specifies which issue fields will be included in the result. <br>
   ...
   *     </ul>
   *
   * @param maxResults if non-<code>null</code>, defines the maximum number of results that can be
   *     returned
   ...
   */
  private static URI createSearchURI(
      RestClient restclient,
      String jql,
      String includedFields,
      String expandFields,
