...
public class RestClient {
  ...
  private JSON request(HttpRequestBase req) throws RestException, IOException {
    ...
    if (creds != null) creds.authenticate(req);
    ...
    if (sl.getStatusCode() >= 300)
      throw new RestException(
          sl.getReasonPhrase(), sl.getStatusCode(), result.toString(), resp.getAllHeaders());
   ...
   * Executes an HTTP POST with the given URI and payload.
   *
   * <p>At least one JIRA REST endpoint expects malformed JSON. The payload argument is quoted and
   * sent to the server with the application/json Content-Type header. You should not use this
   * function when proper JSON is expected.
   *
   * @link https://jira.atlassian.com/browse/JRA-29304
   * @param uri Full URI of the remote endpoint
